...
import {
  splitTeamsByVeteranJovemThenPosition,
  type PlayerGeneration,
...
interface Player {
  ...
  name: string;
  position: string;
  /** Veterano | Jovem — usado na montagem dos times. */
...
interface PredefinedPlayer {
  ...
  defaultGeneration: PlayerGeneration;
}
...
/** Lista base: `defaultPosition` + opcional `generation` (senão = Veterano, só para o algoritmo). */
const PREDEFINED_SOCIETY_RAW: {
  name: string;
  defaultPosition: SocietyPosition;
  generation?: PlayerGeneration;
}[] = [
  ...
  { name: 'Isaque', defaultPosition: 'Ata' },
...
const PREDEFINED_SOCIETY_SEED = PREDEFINED_SOCIETY_RAW.map(row => ({
  ...row,
  generation: row.generation ?? ('Veterano' as PlayerGeneration),
}));

// Lista pré-definida (ordem alfabética por nome)
const initialPredefinedPlayers: PredefinedPlayer[] = [
  ...PREDEFINED_SOCIETY_SEED,
]
  ...
  .map((row, index) => ({
    ...
    defaultGeneration: row.generation,
  }));
...
export default function FootballTeams() {
  ...
  const [newPlayer, setNewPlayer] = useState({
    name: '',
    generation: 'Veterano' as PlayerGeneration,
    position: '',
  });
      ...
      setPredefined(prev => {
        const next = [
          ...prev,
          {
            ...
            defaultGeneration: newPlayer.generation,
          },
        ];
  ...
  const addPredefinedPlayers = () => {
    ...
    const newPlayers: Player[] = selectedPredefinedPlayers.map(playerId => {
      ...
      return {
        ...
        generation: predefinedPlayer.defaultGeneration,
      };
    });
  ...
  const generateTeams = async (
    ...
    toast({
      ...
      description:
        'Montando os times (cada um com a posição cadastrada).',
    });
    ...
    try {
      ...
      const teams = splitTeamsByVeteranJovemThenPosition(
        selected,
          ...
          {societyConfigured && (
            <Card className="bg-gray-50/60 dark:bg-gray-900/60 backdrop-blur-md border-gray-200/30 dark:border-gray-700/30 shadow-xl">
              <CardHeader>
                ...
                <CardDescription className="text-gray-600 dark:text-gray-300 text-sm sm:text-base">
                  Selecione os jogadores que vão participar. Clique no nome para
                  editar; a posição pode ser ajustada ao lado.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
            ...
            <Card className="bg-gray-50/60 dark:bg-gray-900/60 backdrop-blur-md border-gray-200/30 dark:border-gray-700/30 shadow-xl">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-gray-800 dark:text-gray-100 text-lg sm:text-xl">
                  ...
                  Cadastrar Novo Jogador - Society ({numberOfTeams} times,{' '}
                  {playersPerTeam} jogadores cada)
                </CardTitle>
                <CardDescription className="text-gray-600 dark:text-gray-300 text-sm sm:text-base">
                  Nome, geração e posição — na montagem dos times a ordem é
                  primeiro <strong>geração</strong> (veteranos, depois jovens),
                  depois <strong>posição</strong> no campo.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 sm:gap-4">
                  ...
                  <div className="sm:col-span-2 lg:col-span-1">
                    <Label
                      htmlFor="new-player-generation"
                      className="text-gray-700 dark:text-gray-200 text-sm sm:text-base"
                    >
                      Geração
                    </Label>
                    <Select
                      value={newPlayer.generation}
                      onValueChange={value =>
                        setNewPlayer({
                          ...newPlayer,
                          generation: value as PlayerGeneration,
                        })
                      }
                    >
                      <SelectTrigger
                        id="new-player-generation"
                        className="bg-gray-50/50 dark:bg-gray-800/50 backdrop-blur-sm border-gray-200/40 dark:border-gray-600/40"
                      >
                        <SelectValue />
                      </SelectTrigger>
